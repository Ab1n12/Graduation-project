...
public class UserController {
    ...
    @ApiOperation(value = "查询用户")
    public Result<List<User>> getUserAll(@ModelAttribute User user, @ModelAttribute PageVo page) {
        QueryWrapper<User> userQw = new QueryWrapper<>();

       List<User> userData = iUserService.query().list();
        for(User u: userData) {
            QueryWrapper<Role> roleQw = new QueryWrapper<>();
            roleQw.inSql("id","SELECT role_id FROM a_user_role WHERE user_id = '" + u.getId() + "'");
            List<Role> list = iRoleService.list(roleQw);
        ...
        return new ResultUtil<List<User>>().setData(userData);
    }
